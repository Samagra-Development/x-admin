import {
  ...
  import groupDataProvider from './configurableDataProvider.groupUser.package';
  
  const config = require('../../config');
  
  export default (c) => {
    const dataProviderOldData = groupDataProvider(c || config.fusionAuth);
    return {
    ...
    //   update: (resource, params) => dataProviderOldData(UPDATE, resource, params),
      create: (resource, params) => dataProviderOldData(CREATE, resource, params),
        ...
        deleteMany: (resource, params) => dataProviderOldData(DELETE, resource, params),
    };
  };
